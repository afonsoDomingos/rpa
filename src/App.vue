...
const irisColor = ref(
  "radial-gradient(circle at 50% 55%, #c000c0 0%, #800080 90%)"
);
...
const openLinkedIn = () => {
  window.open(
    "https://www.linkedin.com/in/afonso-domingos-6b59361a5/",
    "_blank"
  );
};
...
@keyframes float {
  0%,
  100% {
    transform: translateY(0) scale(4.6);
  }
  50% {
    transform: translateY(-10px) scale(4.6);
  }
}
...
.eye {
  ...
  height: 30px;
  border-radius: 50%/50%;
  background: radial-gradient(circle at 50% 55%, #fff 0%, #f5f5f5 95%);
  position: relative;
  overflow: hidden;
...
.iris {
  ...
  border-radius: 50%;
  position: relative;
  background: v-bind(irisColor);
  box-shadow: 0 0 28px #800080, inset 0 3px 6px rgba(255, 255, 255, 0.4),
...
.iris-fibers {
  ...
  animation: rotateFibers 32s linear infinite;
  opacity: 0.8;
}
.iris-glow {
  position: absolute;
