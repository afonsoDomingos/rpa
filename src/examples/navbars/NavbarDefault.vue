...
<template>
  <nav class="navbar navbar-expand-lg top-0 position-fixed mx-auto my-3"
    style="left: 0; right: 0; max-width: 1200px; border-radius: 12px; z-index: 1030;" :class="{
    ...
    <div class="container px-0">
      ...
      <RouterLink class="navbar-brand animated-logo d-flex align-items-center" :to="{ name: 'presentation' }"
        rel="tooltip" title="Página inicial" @mouseenter="hovering = true" @mouseleave="hovering = false">
        <div class="logo-wrapper">
          <transition name="fade-scale">
            <olhodedeus v-if="!hovering" class="eye-logo" />
          </transition>
          <transition name="fade-scale">
            <img v-if="hovering" src="@/assets/img/rPa.png" alt="Logo Rpa" class="brand-logo" />
          </transition>
        </div>
      </RouterLink>
      ...
      <a @click="$router.push({ name: 'Assinaturas' }); fecharDropdown()"
        class="btn btn-sm mb-0 ms-auto d-lg-none d-block btn-assinatura" role="button">
        ASSINATURAS
      </a>
          ...
          <li v-if="usuario" class="nav-item dropdown dropdown-hover mx-2">
            ...
            <div ref="dropdownMenu"
              ...
              <button v-if="usuario && usuario.role?.toLowerCase() === 'admin'"
                ...
                <i class="bi bi-speedometer2 text-warnin"></i> Dashboard
              </button>
...
body {
  padding-top: 60px;
  /* ajuste conforme a altura real do navbar */
}
...
@media (max-width: 991px) {
  .navbar {
    ...
    max-width: 380px;
    /* evita que fique muito larga em tablets */
    margin: 0 auto;
