<template>
  <div class="meus-anuncios">
        ...
        <div v-for="(ad, i) in anuncios" :key="ad._id" class="anuncio-card" :style="{ '--i': i }">
          ...
          <div class="anuncio-acoes">
            <div class="acoes-linha">
              ...
              <button @click="abrirEstatisticas(ad)" class="btn-stats" type="button">
    ...
    <!-- MODAL DE EDIÇÃO -->
    <div v-if="editAd" class="modal-overlay" @click.self="fecharEdicao" ref="modal" tabindex="0">
      <div class="modal-content" @click.stop>
        ...
        <form @submit.prevent="salvarEdicao" class="grid">
          ...
          <section class="section">
            ...
            <div class="group">
              <input v-model.trim="editForm.name" type="text" class="input" placeholder="Nome do anúncio" required maxlength="100" :class="{ 'error': nameError }" />
              <p v-if="nameError" class="error-text">{{ nameError }}</p>
            </div>

            <div class="group">
              <textarea v-model.trim="editForm.description" class="input textarea" rows="3" placeholder="Descrição detalhada" required maxlength="500" :class="{ 'error': descError }"></textarea>
            ...
            <div class="group">
              <div class="price">
                ...
                <input v-model.number="editForm.price" type="number" min="1" class="input price-input" placeholder="Preço" required :class="{ 'error': priceError }" />
              </div>
            ...
            <div class="group">
              <div class="input-with-icon">
                <input v-model="rawCta" type="text" class="input" placeholder="Número ou wa.me/..." required @blur="formatAndValidateCta" :class="{ 'error': ctaError, 'valid': !ctaError && rawCta }" />
              ...
              <p v-else-if="editForm.ctaLink" class="success-text">
                <i class="bi bi-whatsapp"></i> Link: <a :href="editForm.ctaLink" target="_blank" rel="noopener">{{ editForm.ctaLink }}</a>
              </p>
            </div>
          </section>
          ...
          <section class="section">
            ...
            <div class="group">
              ...
              <input type="file" accept="image/jpeg,image/png,image/webp" @change="onFileChange" id="file-edit" class="hidden" />
          ...
          <button type="submit" class="btn" :disabled="editLoading || !isFormValid" :class="{ 'btn-active': isFormValid && !editLoading }">
    ...
    <div v-if="statsModal" class="modal-overlay" @click.self="fecharEstatisticas">
      <div class="modal-content stats-modal">
        ...
        <div class="stats-grid">
          <div class="stat-card"><i class="bi bi-eye"></i><div><p>Visualizações</p><strong>{{ stats?.views || 0 }}</strong></div></div>
          <div class="stat-card"><i class="bi bi-hand-index-thumb"></i><div><p>Cliques</p><strong>{{ stats?.clicks || 0 }}</strong></div></div>
          <div class="stat-card"><i class="bi bi-percent"></i><div><p>CTR</p><strong>{{ stats?.ctr || '0%' }}</strong></div></div>
          <div class="stat-card today"><i class="bi bi-calendar-today"></i><div><p>Cliques Hoje</p><strong>{{ stats?.hoje || 0 }}</strong></div></div>
        </div>
        ...
        <button @click="fecharEstatisticas" class="btn-close-stats">Fechar</button>
      </div>
    </div>
  </div>
</template>
...
const editForm = ref({ id: '', name: '', description: '', price: 0, ctaLink: '', image: null, imageUrl: '' })
const rawCta = ref('')
...
// Funções auxiliares
const formatPrice = (value) => new Intl.NumberFormat('pt-MZ', { style: 'currency', currency: 'MZN', minimumFractionDigits: 0 }).format(value)
const getStatusText = (status) => ({ active: 'Ativo', pending: 'Pendente', paused: 'Pausado' }[status] || 'Indefinido')
...
const carregarAnuncios = async () => {
  try {
    ...
    const res = await api.get('/anuncios/meus', { headers: { Authorization: `Bearer ${token}` } })
...
const recarregar = () => { loading.value = true; error.value = ''; carregarAnuncios() }

// Remover anúncio
const confirmarRemocao = async (id) => {
  if (!confirm('Tem certeza que deseja remover este anúncio?')) return
  try {
    await api.delete(`/anuncios/${id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } })
...
watch(() => editForm.value.name, (val) => { nameError.value = val?.trim().length < 3 ? 'Mínimo 3 caracteres' : '' })
watch(() => editForm.value.description, (val) => { descError.value = val?.trim().length < 10 ? 'Mínimo 10 caracteres' : '' })
watch(() => editForm.value.price, (val) => { priceError.value = val < 1 ? 'Preço deve ser maior que 0' : '' })

const formatAndValidateCta = () => {
  const input = rawCta.value.trim()
  const urlRegex = /^https?:\/\/(wa\.me|api\.whatsapp\.com|chat\.whatsapp\.com)\//i
  if (urlRegex.test(input)) { editForm.value.ctaLink = input; ctaError.value = ''; return }
...
const isFormValid = computed(() => {
  ...
  const validCta = /^https?:\/\/wa\.me\/[0-9]+$/.test(editForm.value.ctaLink.trim())
...
const salvarEdicao = async () => {
  if (!isFormValid.value) return (submitError.value = 'Preencha todos os campos corretamente.')
  ...
  try {
    ...
    if (index !== -1) {
      anuncios.value[index] = {
        ...
        image: editForm.value.image ? res.data.image : anuncios.value[index].image
      }
    }
...
const criarGrafico = () => {
  ...
  const labels = stats.value.ultimoSeteDias.map(item => new Date(item.date).toLocaleDateString('pt-MZ', { weekday: 'short', day: 'numeric' }))
  const data = stats.value.ultimoSeteDias.map(item => item.clicks)

  chartInstance.value = new Chart(chartCanvas.value, {
    type: 'bar',
    data: { labels, datasets: [{ label: 'Cliques', data, backgroundColor: '#7c3aed', borderRadius: 8 }] },
    options: {
      responsive: true,
...
.loading-state, .error-state, .empty-state { text-align: center; margin-top: 6rem; }
.loading-state i, .error-state i, .empty-state i { font-size: 2.5rem; margin-bottom: 1rem; display: block; }
...
.retry-btn, .new-btn { margin-top: 1rem; background: #7c3aed; color: #fff; padding: 0.7rem 1.4rem; border-radius: 10px; border: none; cursor: pointer; font-weight: 600; }
.retry-btn:hover, .new-btn:hover { background: #6d28d9; transform: translateY(-1px); }

.header { display: flex; align-items: center; gap: 1rem; margin-bottom: 2rem; }
.page-title { font-size: 1.6rem; font-weight: 700; color: #fff; margin: 0; }
.back-btn { background: none; border: 1px solid rgba(255,255,255,0.3); padding: 0.5rem 1rem; border-radius: 8px; color: #fff; cursor: pointer; display: flex; align-items: center; gap: 0.5rem; }
.back-btn:hover { background: rgba(255,255,255,0.1); }

.grid-anuncios { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 1.8rem; }

.anuncio-card {
  position: relative; background: rgba(255,255,255,0.05); backdrop-filter: blur(18px);
  border: 1px solid rgba(255,255,255,0.15); border-radius: 1rem; padding: 1rem;
  transition: all 0.3s ease; opacity: 0; transform: translateY(20px);
  animation: fadeUp 0.6s ease forwards; animation-delay: calc(0.1s * var(--i));
}
.anuncio-card:hover { transform: translateY(-6px); box-shadow: 0 12px 32px rgba(0,0,0,0.35); }

.anuncio-status { position: absolute; top: 0.8rem; right: 0.8rem; padding: 0.25rem 0.7rem; border-radius: 1rem; font-size: 0.7rem; font-weight: 600; text-transform: uppercase; }
...
.anuncio-img { width: 100%; height: 180px; object-fit: cover; border-radius: 0.8rem; margin-bottom: 1rem; background: #1a1a1a; opacity: 0; transition: opacity 0.4s; }
.anuncio-img.loaded { opacity: 1; }

.anuncio-titulo { font-size: 1.1rem; font-weight: 600; margin-bottom: 0.3rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: #fff; }
.anuncio-desc { font-size: 0.9rem; color: #d9d9d9; margin-bottom: 0.6rem; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
.anuncio-preco { font-weight: 700; font-size: 1.1rem; color: #81c784; display: flex; align-items: center; gap: 0.3rem; margin-bottom: 1rem; }

/* BOTÕES PERFEITOS */
.anuncio-acoes { margin-top: 1rem; display: flex; flex-direction: column; gap: 0.6rem; padding-top: 0.8rem; border-top: 1px solid rgba(255,255,255,0.1); }
.acoes-linha { display: flex; gap: 0.6rem; }
.acoes-linha button, .acoes-linha a {
  flex: 1; padding: 0.7rem 0.5rem; border-radius: 10px; border: none; font-size: 0.875rem; font-weight: 600;
  cursor: pointer; transition: all 0.3s ease; display: flex; align-items: center; justify-content: center; gap: 0.5rem; min-height: 44px;
}
.btn-contato { background: linear-gradient(135deg, #7c3aed, #9333ea); color: white; box-shadow: 0 4px 12px rgba(124,58,237,0.3); }
.btn-contato:hover { background: #25d366; transform: translateY(-2px); box-shadow: 0 6px 16px rgba(37,211,102,0.4); }
.btn-stats { background: rgba(124,58,237,0.15); color: #e0d4ff; border: 1px solid rgba(124,58,237,0.3); }
.btn-stats:hover { background: #7c3aed; color: white; transform: translateY(-2px); }
.btn-editar { background: rgba(255,255,255,0.1); color: #fff; }
.btn-editar:hover { background: rgba(255,255,255,0.2); transform: translateY(-2px); }
.btn-remover { background: rgba(239,68,68,0.15); color: #ff6b6b; border: 1px solid rgba(239,68,68,0.3); }
.btn-remover:hover { background: rgba(239,68,68,0.3); color: #ff4444; transform: translateY(-2px); }

@media (max-width: 480px) { .acoes-linha { flex-direction: column; } }

@keyframes fadeUp { to { opacity: 1; transform: translateY(0); } }

/* MODAIS */
.modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.7); display: flex; align-items: center; justify-content: center; z-index: 1000; padding: 1rem; }
.modal-content { background: #fff; border-radius: 1rem; padding: 2rem; max-width: 800px; width: 100%; max-height: 90vh; overflow-y: auto; position: relative; color: #000; }
...
.stats-modal { max-width: 600px; text-align: center; }
.stats-title { font-size: 1.5rem; margin-bottom: 0.5rem; color: #7c3aed; }
.stats-name { font-size: 1.1rem; color: #666; margin-bottom: 1.5rem; }
.stats-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem; margin-bottom: 2rem; }
.stat-card { background: #f8f9fa; padding: 1rem; border-radius: 1rem; display: flex; align-items: center; gap: 1rem; color: #333; }
.stat-card i { font-size: 2rem; color: #7c3aed; }
.stat-card.today { background: #f0e8ff; }
.stat-card strong { font-size: 1.5rem; display: block; }
.chart-container { margin: 2rem 0; padding: 1rem; background: #f8f9fa; border-radius: 1rem; height: 300px; }
.btn-close-stats { margin-top: 1rem; padding: 0.8rem 2rem; background: #7c3aed; color: #fff; border: none; border-radius: 0.5rem; cursor: pointer; }
...
.icon, .icon-x { width: 2rem; height: 2rem; stroke: currentColor; }
