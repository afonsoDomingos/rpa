...
watch(
  ...
  () => {
    const targetDocs = filtroProvincia.value
      ? documentos.value.filter((d) => d.provincia === filtroProvincia.value)
          .length
  ...
  datasets: [
    {
      ...
      data: rankingDiscreto.value.map(
        (i) => populacaoProvincias[i.provincia]?.homens || 0
      ),
    },
    {
      backgroundColor: "#ec4899",
      data: rankingDiscreto.value.map(
        (i) => populacaoProvincias[i.provincia]?.mulheres || 0
      ),
    },
    {
      backgroundColor: "#10b981",
      data: rankingDiscreto.value.map((i) => i.docs),
    },
  ],
}));
...
const desenharMarcadores = () => {
  ...
  rankingDiscreto.value.forEach((item) => {
    ...
    L.circleMarker(c, {
      ...
      .bindPopup(
        `<div style="background:#000;color:#fff;padding:8px 11px;border-radius:6px;font-family:Poppins;font-weight:600;font-size:13px"><b>${item.nomeCurto}</b><br>${item.docs} docs</div>`
      )
      .addTo(markersLayer);
  });
  if (filtroProvincia.value)
    nextTick(() =>
      mapa.value.setView(coordenadasProvincias[filtroProvincia.value], 10)
    );
};

const centrarProvincia = (p) =>
  coordenadasProvincias[p] && mapa.value.setView(coordenadasProvincias[p], 10);
  ...
  <div class="app">
    ...
    <div class="painel">
      ...
      <div class="filtro">
        ...
        <button v-if="filtroProvincia" @click="limparFiltro" class="limpar">
          ×
        </button>
      </div>
      ...
      <div class="ranking">
        <div
          v-for="(item, i) in rankingDiscreto"
          :key="item.provincia"
          class="rank-line"
        >
          ...
          <button @click="centrarProvincia(item.provincia)" class="ver">
            Ver
          </button>
        </div>
      </div>
