...
const filtradas = computed(() => {
  return solicitacoes.value.filter(item =>
      ...
      <table class="table table-hover shadow-sm">
        ...
        <tbody>
          <tr v-for="item in paginadas" :key="item._id || item.id">
            ...
            <td>
              <button class="btn btn-sm btn-outline-primary" @click="selecionarSolicitante(item)">
                Ver Detalhes
              </button>
            </td>
          </tr>
...
<div v-if="solicitanteSelecionado" class="mt-5">
  <div class="text-center mb-4">
    ...
    <p class="text-muted">Informações completas do solicitante selecionado.</p>
  </div>
    ...
    <div class="card-body">
      <div class="row g-3">
        <div class="col-md-6">
          ...
          <div class="form-control bg-light">{{ solicitanteSelecionado.nome_completo }}</div>
        </div>
        <div class="col-md-6">
          <label class="form-label fw-semibold">Tipo de Documento:</label>
          <div class="form-control bg-light">{{ solicitanteSelecionado.tipo_documento }}</div>
        </div>
        <div class="col-md-6">
          <label class="form-label fw-semibold">Contacto:</label>
          <div class="form-control bg-light">{{ solicitanteSelecionado.contacto }}</div>
        </div>
        <div class="col-md-6">
          <label class="form-label fw-semibold">Afiliação:</label>
          <div class="form-control bg-light">{{ solicitanteSelecionado.afiliacao || '-' }}</div>
        </div>
        <div class="col-md-6">
          <label class="form-label fw-semibold">Local de Emissão:</label>
          <div class="form-control bg-light">{{ solicitanteSelecionado.local_emissao || '-' }}</div>
        </div>
        <div class="col-md-6">
          <label class="form-label fw-semibold">Data de Nascimento:</label>
          <div class="form-control bg-light">{{ solicitanteSelecionado.data_nascimento || '-' }}</div>
        </div>
        <div class="col-md-6">
          <label class="form-label fw-semibold">Número do BI:</label>
          <div class="form-control bg-light">{{ solicitanteSelecionado.numero_bi || '-' }}</div>
        </div>
        <div class="col-md-6">
          <label class="form-label fw-semibold">Motivo da Solicitação:</label>
          <div class="form-control bg-light">{{ solicitanteSelecionado.motivo || '-' }}</div>
        </div>
      </div>
    </div>
  </div>
